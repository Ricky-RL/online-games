...
export function createInitialTower(): JengaGameState {
  ...
  for (let row = 0; row < INITIAL_ROWS; row++) {
    ...
    for (let col = 0; col < BLOCKS_PER_ROW; col++) {
      blocks.push({ id: `${row}-${col}`, exists: false });
    }
    tower.push(blocks);
  }
...
- [ ] **Step 2: Record match result on game end in useJengaGame**
...
```bash
git add src/lib/match-results.ts src/hooks/useJengaGame.ts
