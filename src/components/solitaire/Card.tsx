...
export function SolitaireCard({ cardId, faceUp, draggable = false, dragData, onClick, style, className = '' }: CardProps) {
  ...
  if (!faceUp) {
    return (
      <div
        className={`w-[48px] h-[67px] sm:w-[60px] sm:h-[84px] rounded-lg border border-border bg-gradient-to-br from-blue-900 to-blue-700 shadow-sm ${className}`}
        style={style}
      >
        <div className="w-full h-full rounded-lg border border-blue-600/30 flex items-center justify-center">
          <div className="w-6 h-9 sm:w-8 sm:h-12 rounded border border-blue-400/20 bg-blue-800/50" />
        </div>
      </div>
    );
  }
...
export function EmptySlot({ label, className = '' }: { label?: string; className?: string }) {
  return (
    <div className={`w-[48px] h-[67px] sm:w-[60px] sm:h-[84px] rounded-lg border-2 border-dashed border-border/40 flex items-center justify-center ${className}`}>
