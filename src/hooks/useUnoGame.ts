...
import {
  ...
  drawCardForTurn,
  passAfterDraw,
  playUnoCard as computePlayUnoCard,
  type UnoBoardState,
  type UnoColor,
} from '@/lib/uno-logic';
...
function getMyPlayerNumber(game: UnoGame): Player | null {
...
export function useUnoGame(gameId: string): UseUnoGameReturn {
  ...
  const updateGame = useCallback((updater: UnoGame | null | ((prev: UnoGame | null) => UnoGame | null)) => {
    setGame((prev) => {
  ...
  const recordWin = useCallback((currentGame: UnoGame, winner: Player, board: UnoBoardState) => {
  ...
  const persistState = useCallback(async (nextBoard: UnoBoardState, winner: Player | null, previousGame: UnoGame) => {
    ...
    optimisticTurnSequence.current = null;
    updateGame(updatedGame as UnoGame);
    if (winner) recordWin(previousGame, winner, nextBoard);
  }, [fetchGame, gameId, recordWin, updateGame]);

  const playCard = useCallback(async (cardId: string, chosenColor?: UnoColor) => {
    ...
    try {
      const result = computePlayUnoCard(current.board, myPlayer, cardId, chosenColor);
      ...
      if (!opponentJoined) {
        ...
        if (result.board.activePlayer === myPlayer) {
          setError('Wait for opponent to join before using skip/reverse/draw actions');
          return;
        }
      }
  ...
  const resetGame = useCallback(async () => {
    const newBoard = createUnoBoard(1);
  ...
  const endGame = useCallback(async () => {
    ...
    if (deleteError) {
      console.error('Error deleting UNO game:', deleteError);
    }
    setDeleted(true);
  }, [gameId]);
