...
export default function UnoLobby() {
  ...
  const connect = useCallback(async (name: PlayerName) => {
    setConnecting(name);
    ...
    async function joinGame(gameId: string) {
      ...
      if (error) {
        console.error('Error joining UNO:', error);
        setConnecting(null);
        return false;
      }
      return true;
    ...
    if (activeGame) {
      router.push(`/uno/${activeGame.id}`);
      return;
    }

    if (joinableGame) {
      if (await joinGame(joinableGame.id)) router.push(`/uno/${joinableGame.id}`);
      return;
    }
    ...
    if (error || !data) {
      console.error('Error creating UNO game:', error);
      setConnecting(null);
      return;
    }

    router.push(`/uno/${data.id}`);
  }, [router]);
