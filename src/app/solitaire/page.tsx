    ...
    async (name: PlayerName) => {
      ...
      async function joinGame(gameId: string) {
        const { error: joinError } = await supabase
          .from('games')
          .update({ player2_id: myId, player2_name: name, updated_at: new Date().toISOString() })
          .eq('id', gameId)
          .select()
          .single();
      ...
      if (activeGame) { router.push(`/solitaire/${activeGame.id}`); return; }
      if (joinableGame) { if (await joinGame(joinableGame.id)) router.push(`/solitaire/${joinableGame.id}`); return; }
    ...
    <div className="flex-1 flex flex-col items-center justify-center px-6">
      <main className="flex flex-col items-center gap-12 text-center">
        ...
        <div className="space-y-3">
          <h1 className="text-5xl sm:text-6xl font-bold tracking-tight text-text-primary">Solitaire</h1>
        ...
        <button onClick={() => router.push('/')} className="text-sm text-text-secondary hover:text-text-primary transition-colors cursor-pointer">
          Back to games
        </button>
      </main>
    </div>
