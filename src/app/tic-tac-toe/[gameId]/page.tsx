...
function getGameStatus(game: TicTacToeGame): GameStatus {
...
export default function TicTacToeGamePage({ params }: { params: Promise<{ gameId: string }> }) {
  const { gameId } = use(params);
  const { game, loading, error, lastMove, deleted, makeMove, resetGame } = useTicTacToeGame(gameId);
  ...
  const winningCells = useMemo((): [number, number][] | null => {
    ...
    return getWinningCells(game.board);
  }, [game]);
